...
app.certifica = kendo.observable({
    onShow: function () {
        try {
            ...
            registroavaluo = "";
            document.getElementById('infoPlacasAvaluo').value = "";
            llamarCERT();
...
function llamarCerti(placaAvaluo) {
    kendo.ui.progress($("#lector_barrasScreen"), true);
    setTimeout(function () {
       consultaCertPlaca(placaAvaluo.toUpperCase());
    }, 2000)    ;
}
    ...
    try {
        ...
        if (infVINResp.length > 0) {
            ...
            $("#gridCertifica").kendoGrid({
                dataSource: {
                    data: infVINResp,
                    sort:{ field: "fecha_registro", dir: "desc" },
                    pageSize: 20
                },
                // height: 400,
                scrollable: false,
                            ...
                            click: function (e) {
                                try {
                                    ...
                                    registroavaluo = dataItem;
                                        kendo.mobile.application.navigate("components/estado_certificacion/view.html");
...
function consultaCertPlaca(placaAva) {
    var infVINResp = "";
    try {
        localStorage.removeItem("registroGe");
        var grid01_1 = $("#gridCertifica").data("kendoGrid");
        grid01_1.destroy();
    } catch (error) {
    ...
    try {
        //http://192.168.1.50:8089/concesionario/Services/VH/Vehiculos.svc/vh62VinUsadoGet/4,json;1;01;01;;;;;;;;AVALUADO
        var UrlVIN = localStorage.getItem("ls_url2").toLocaleString() + "/Services/VH/Vehiculos.svc/vh62VinUsadoGet/4,json;1;01;01;"+placaAva+";;;;;;;AVALUADO";
        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", UrlVIN);
